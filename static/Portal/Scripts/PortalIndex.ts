...
// Component Imports
import type { CalendarChangeEvent } from '@progress/kendo-vue-dateinputs'
import { Calendar } from '@progress/kendo-vue-dateinputs'
import type { SchedulerDataItem } from '@ExemplarComponents/Scheduler'
import Scheduler from '@ExemplarComponents/Scheduler'
...
import type { AdjusterCounts } from './ViewModels/AdjusterCounts'
import type { AdjusterProfile } from './ViewModels/AdjusterProfile'
...
export default class PortalIndexComponent extends PaginatedPageBase<AdjusterInspectionsGrid> {
  ...
  async RefreshAccessToken() {
    const refreshTokenUrl: string = `${$('#exemplarCoreUrl').val()}Authorization/GetRefreshAccessToken`
    const newaccessToken = await ApiClient.Get(refreshTokenUrl)
  ...
  onCountsClicked(projectStatusId: number, projectStatusName: string) {
    ...
    const snapshotReadRoute = `AdjusterPortal/${this.currentUser.UserId}?MappingType=1&RoleId=${this.adjusterRoleId}&ProjectStatusId=${this.projectStatusId}&ScheduleDate=${this.selectedDate}`
