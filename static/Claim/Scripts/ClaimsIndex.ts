...
import { GlobalEventBus, GlobalEvents } from '@ExemplarCommon/eventBus'
...
import type { Customer } from '@ExemplarViewModels/Customer'
...
import type ClaimValidationError from '@ExemplarViewModels/ClaimValidationError'
...
import type { CompanyServiceType } from '@ExemplarViewModels/CompanyServiceType'
import type { ExternalClaimFormAdjuster } from '@ExemplarViewModels/ExternalClaimFormAdjuster'

import type { DatePickerChangeEvent } from '@progress/kendo-vue-dateinputs'
...
import type { AvailableRoutes } from '../../../Scripts/ViewModels/AvailableRoutes'
import type ServiceAreaPostalCode from '../../../Scripts/ViewModels/ServiceAreaPostalCode'
...
export default class ClaimsIndexComponent extends EditFormBase<ExternalWebForm> {
  ...
  initAutocomplete() {
    const test = document.getElementById('street-address1')
  ...
  async GetAdjuster() {
    const route = `${this.exemplarCoreUrl}Claim/Claims/GetAdjusterById?adjusterId=${this.adjusterId}`

    await this.GetModel(route, this.GetAdjusterSuccessfullCallback, null, false, true, this.customErrorMessage)
  }

  async GetServiceTypes() {
    const route = `${this.exemplarCoreUrl}Claim/Claims/GetExternalFormCompanyServiceTypes?companyId=${this.editModel.CompanyId}`
  ...
  GetAdjusterSuccessfullCallback(model: any) {
    const externalClaimFormAdjuster: ExternalClaimFormAdjuster = model.Model
  ...
  CheckIfAdjusterEmailMatches() {
    if (this.editModel.AdjusterEmail && this.editModel.AdjusterConfirmEmail) {
      if (this.editModel.AdjusterEmail.toLowerCase().trim() === this.editModel.AdjusterConfirmEmail.toLowerCase().trim()) {
        ...
        const editForm = this.$refs.editFormRef as EditFormComponent<ExternalWebForm>
        if (editForm) {
          const idx = editForm.validationResult.findIndex((x: string) => x === 'Please confirm that the entered Adjuster\'s email matches in both text boxes.')
          if (idx >= 0)
    ...
    else { this.showAdjusterMessage = false }
  }

  CreateExternalServiceType(companyServiceType: CompanyServiceType): ExternalClaimCompanyServiceType {
    const externalClaimCompanyServiceType: ExternalClaimCompanyServiceType = new ExternalClaimCompanyServiceType()
  ...
  DamageTypeExists(damageTypeId: number): boolean {
    const damageType = this.editModel.SelectedDamageTypes.find((damageType: ProjectDamageTypeInsert) => damageType.DamageTypeId == damageTypeId)
  ...
  ProcessDamageTypes(damageTypeId: number) {
    ...
    if (damageType) {
      ...
      const newDamageType: ProjectDamageTypeInsert = new ProjectDamageTypeInsert()
  ...
  GetOtherValue() {
    const damageType = this.editModel.SelectedDamageTypes.find((damageType: ProjectDamageTypeInsert) => damageType.DamageTypeId == 4)
    if (damageType)
  ...
  RequestCoverage() {
    ...
    const requestModel = { PostalCode: this.editModel.PostalCode }
  ...
  async GetAdjusterInfo() {
    const url = `${this.exemplarApiUrl}PaymentGateway/GetCustomerProfile?id=72087`
  ...
  PreSaveCallback() {
    ...
    if (this.otherCompanyServiceTypes && this.otherCompanyServiceTypes.length > 0) {
      for (var companyServiceType of this.editModel.SelectedCompanyServiceTypes) {
        const idx = this.otherCompanyServiceTypes.findIndex((x: CompanyServiceType) => { return x.AvailableServiceTypeId === companyServiceType.AvailableServiceTypeId })
        if (idx < 0) {
  ...
  GetServiceTypesSuccessfullCallback(model: any) {
    ...
    const listOfServiceTypes = model.Model
  ...
  GetCompanyByNameCallback(model: any) {
    if (model.Model) {
      ...
      const company: Customer = model.Model
  ...
  async SaveCallBack(model: any) {
    ...
    if (!this.IsAdditionalClaim) {
      ...
      const href = `${this.exemplarCoreUrl + ExemplarPageRoutes.Claim}?AdjusterId=${model.Model.Project.AdjusterId}&FromPortal=false`
  ...
  ParseAppointmentDateAndTime(selectedTime: string) {
    ...
    const minutes: string = selectedTime.slice(2, 4)

    const hour: string = selectedTime.slice(0, 2)

    const scheduleDate = `${this.editModel.AppointmentDate} ${hour}:${minutes}`
  ...
  ValidateClaim() {
    if (this.claimFormat) {
      const [example, regex] = this.claimFormat.split('|')
  ...
  OnAppointmentDateChange(event: DatePickerChangeEvent): void {
    ...
    const inspectionDateString = `${event.value!.getFullYear()}-${event.value!.getMonth() + 1}-${event.value!.getDate()}`

    this.editModel.AppointmentDate = inspectionDateString

    // @ts-expect-error
    const year: string = this.appointmentDate.getFullYear().toString()

    // @ts-expect-error
    if (this.editModel.AppointmentDate.length < 8 || year.length < 4)
      return

    const minD: Date = new Date(this.minDate())
    ...
    if (this.editModel.ProjectInspection.ServiceAreaId > 0) {
      const zipRoute = `${this.exemplarApiUrl}ExternalWebForm/AreRoutesAvailable?inspectionDate=${this.editModel.AppointmentDate}&serviceAreaId=${this.editModel.ProjectInspection.ServiceAreaId}`
  ...
  OnServiceTypeChecked(availableServiceTypeId: number) {
    ...
    if (this.SelectedServiceTypeIds.includes(availableServiceTypeId)) {
      const index = this.SelectedServiceTypeIds.indexOf(availableServiceTypeId)
      if (index !== -1) {
        this.SelectedServiceTypeIds.splice(index, 1)

        const serviceType = this.editModel.SelectedCompanyServiceTypes.find((serviceType: CompanyServiceType) => serviceType.AvailableServiceTypeId == availableServiceTypeId)
      ...
      const serviceType = this.companyServiceTypes.find((serviceType: CompanyServiceType) => serviceType.AvailableServiceTypeId == availableServiceTypeId)
    ...
    const directInspection = this.directInspectionIds.includes(availableServiceTypeId)
    const ladderAssist = this.SelectedServiceTypeIds.includes(37)
    ...
    if (directInspection && !this.showDamageType.includes(availableServiceTypeId)) {
      ...
      const index = this.showDamageType.indexOf(availableServiceTypeId)
    ...
    const requiresDate = this.editModel.SelectedCompanyServiceTypes.find((serviceType: CompanyServiceType) => serviceType.RequiresDate == true)
    const showCalendar = this.editModel.SelectedCompanyServiceTypes.find((serviceType: CompanyServiceType) => serviceType.ShowCalendar == true)
  ...
  async OnOtherCompanyNameChange(event: any) {
    ...
    const route = `${this.exemplarCoreUrl}Claim/Claims/GetCompanyByCompanyName?companyName=${this.editModel.CompanyName}`
  ...
  async OnCompanyChange(e: any) {
    ...
    if (this.editModel.CompanyId === 384) {
      const route = `${this.exemplarCoreUrl}Claim/Claims/GetCompanyById?companyId=${this.editModel.CompanyId}`
  ...
  ValidateClaimSpecificEmail() {
    if (this.editModel.ClaimSpecificEmail != null && this.editModel.ClaimSpecificEmail != '') {
      const email = this.editModel.ClaimSpecificEmail.toLowerCase().trim()
      const regex = /^[a-z0-9-.]+@claims.usaa.com$/
