...
import type ProjectDamageTypeInsert from '@ExemplarViewModels/ProjectDamageTypeInsert'
...
export class EditFormValidator implements IValidatorListAsync<ExternalWebForm> {
  ...
  CheckProjectInspection(model: ExternalWebForm) {
    const requiresDate = model.SelectedCompanyServiceTypes.find((serviceType: CompanyServiceType) => serviceType.RequiresDate == true)
  ...
  CheckDamageTypes(model: ExternalWebForm) {
    ...
    if (directInspectionSelected && !ladderAssistSelected) {
      if (model.SelectedDamageTypes.length === 0) {
        ...
        const damageType = model.SelectedDamageTypes.find((damageType: ProjectDamageTypeInsert) => damageType.DamageTypeId == 4 && damageType.OtherInspection.length == 0)
