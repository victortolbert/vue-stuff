namespace Exemplar.Web.Areas.ServiceType.Controllers
{
    ...
    using AutoMapper;
    using Exemplar.Dto;
    using Exemplar.Dto.Project;
    using Exemplar.Services.DataAccessService;
    using Exemplar.Services.ExemplarMessageService;
    using Exemplar.Services.NewProjectService;
    using Exemplar.Web.Areas.Claim.Helpers;
    ...
    public class ClaimsController : BaseController
    {
        ...
        public ClaimsController(IExemplarMessageService exemplarMessageService, IOptions<AppSettings> appSettings, IConfiguration configuration, IHttpClientFactory httpClientFactory, NewProjectService newProjectService)
        : base(httpClientFactory, exemplarMessageService, configuration, appSettings)
        {
            ...
            serviceAreaCapacityAlertRecipients = configuration["ServiceAreaCapacityAlertRecipients"] ?? configuration.GetValue<string>("AppSettings:ServiceAreaCapacityAlertRecipients");
            vipEmailRecipients = configuration["vipEmailRecipients"] ?? configuration.GetValue<string>("AppSettings:vipEmailRecipients");
        }
        ...
        public async Task<IActionResult> SubmitClaim([FromBody] string model)
        {
            try
            {
                ...
                if (claimValidationResult.ValidationPass)
                {
                    ...
                    var claimSubmittedResult = await newProjectService.ProcessClaimAsync(newClaimWebFormDto, GoogleDirectionsApiKey, userId, serviceAreaCapacityAlertRecipients, vipEmailRecipients);
            ...
            {
                await ExemplarMessageService.InsertExceptionMessageAsync(GetType().ToString(), GetCurrentMethod(), string.Empty, string.Empty, ex);
